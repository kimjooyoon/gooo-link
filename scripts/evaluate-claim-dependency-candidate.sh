...
jq -S -n \
  ...
  ($r.commit_valid==true and $r.source_valid==true and $r.document_valid==true and $r.activities==6 and $r.graph_edges==8 and $r.recoverable_roots==1 and $r.typed_edge_programs==5 and $r.requires_programs==2 and $r.supports_programs==1 and $r.contradicts_programs==1 and $r.failure_entailment_programs==1) as $research_exact |
